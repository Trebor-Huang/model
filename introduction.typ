...
我们先从具体的例子出发，思考模型的定义应该满足什么条件。最直观的模型是 #[@sec:set-model]中的集合模型。因此，需要保证模型的定义能让集合构成模型。我们据此在 #[@sec:model-definition]中给出模型的定义。
...
在 #[@sec:explicit-substitution]中，我们提到不应该将语境的长度视为本质属性，因此变量代换需要视作新的语法构造，而非在语法上递归定义的函数。在模型的定义中这样的好处是明显的：我们将表达式解释为数学对象之后，递归定义的代换就行不通了。因为两个语法上不同的表达式在模型中可能对应相同的数学对象，因此在这套模型中就不能靠递归定义这两个表达式的代换.#footnote[那么，干脆去掉代换的概念，能否行得通呢? 类型论里有许多规则需要用代换来表达，例如 $beta$ 判值相等 $(lambda x bind t) s = t[s\/x]$。如果去掉代换，这些规则也难以表述。]
...
$)
注意使用了代换 $[id, a] : Gamma -> (Gamma dot A)$ 使得类型正确，表示语境中的其他变量不改变，而将最后一个变量换为 $a$。用具名变量的语言，就是 $B[a\/x]$。在模型中，这就对应二元运算 $"pair"(a,b)$，将 $a in "Tm"(Gamma, A)$ 与 $b in "Tm"(Gamma, B[id, a])$ 映射到 $"Tm"(Gamma, Sigma A B)$。
...
==== 相等类型
给定语义类型 $A in "Tp"(Gamma)$ 与两个元素 $s, t in "Tm"(Gamma, A)$，需要给出相等类型 $"Id"(A, s, t) in "Tm"(Gamma, A)$，满足 $"Id"(A, s, t) sigma = "Id"(A sigma, s sigma, t sigma)$。相等类型的构造子是 $refl_A (t)$，将 $t in "Tm"(Gamma, A)$ 映射到 $"Tm"(Gamma, "Id"(A, t, t))$，满足 $refl_A (t) sigma = refl_(A sigma) (t sigma)$。
...
相等类型同样一般只有 $beta$ 等式：$"J"_A (refl_A (t), P, r) = r[t\/z]$。$eta$ 等式的形式比较复杂，读者可以试着写一写。不过，我们可以考虑另两条比较简单的规则：
#eq($
  rule(
    Gamma tack s = t : A,
    Gamma tack p : "Id"(A, s, t)
...
如果没有外延性质，也可以避免验证 J 消去子。 @sec:J-equivalences 中记录了一套与 J 消去子等价的规则。在构造模型时，只要给出这些规则对应的语义对象，就可以表达出 J。
...
如本章开头所言，一套模型需要使语义对象的性质可以与语法相对应。在模型的定义中，我们保证了搭建语法的_每块积木_都有符合要求的语义构造。因此将其组合起来，就应该得到完整的语法对象对应的语义对象。这就是语义的#define[可靠性][soundness] 定理。
#theorem[可靠性][
