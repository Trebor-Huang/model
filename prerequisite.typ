...
等等。这种定义要求所有的语境都有明确的层次结构：如果 $Gamma = Delta$，那么这两个语境长度必须相同。如果 $(Gamma, x : A) = (Gamma', x' : A')$，那么除了变量名可以修改之外，必须满足 $Gamma = Gamma'$ 与 $A = A'$。否则，代换的递归定义就无法进行。这样，我们可以认为某条规则
#eq($
...
对于两个集合 $A$ 与 $B$，可以靠单射或满射比较元素的多少。如果从 $A$ 到 $B$ 有单射，就说 $abs(A) <= abs(B)$; 如果从 $B$ 到 $A$ 有满射或者 $A$ 是空集，就说 $abs(A) scripts(<=)^* abs(B)$。假定选择公理，可以证明这两者是一样的。同时，可以说明 $abs(A) <= abs(B)$ 与 $abs(A) >= abs(B)$ 同时成立时有 $abs(A) = abs(B)$，并且对于任何两个集合，这三者之一必然成立。这样，全体基数就从小到大排列成一条线。
