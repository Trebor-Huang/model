...
这两种解释办法定义得到语法本质上是等价的，并且都自然地对应一种模型的定义。前者为每一种长度的语境都各自赋予一类对象，而后者将所有的语境放在一起。但是，数学实际中遇到的对象往往不会具有这样明确的长度结构。例如在集合中，$A times nothing = nothing$，因此没有办法区分一个空集究竟是几个集合的乘积。我们故而采用第二种观点，而将第一种观点下的相关定义称作#define[语境性][contextual] 的。不过，这两者实际上大同小异，只是细节处理上后者会更加优雅。
...
元素关系与累积关系是互相独立的，即两两组合一共有四种可能性。例如 Rocq (Coq) 证明助理中 $"Prop"$ 就是 $"Set"$ 的子类型，但是 $"Prop" : "Set"$ 不成立。
...
为了实现依值类型论的类型检查，我们需要能够判定表达式之间是否有判值相等。 这可以依靠#translate[正规形式][normal form] 实现。如果每个表达式都等于唯一一个正规形式，那么只要计算出正规形式并比较即可。传统中，往往可以在表达式上定义一系列重写关系，例如 $1 + 1 ~> 2$ 或者 $pi_1 (a, b) ~> a$，并定义正规形式为不能通过重写化简的表达式，不过这不是必须的。对于常见的依值类型论而言，可以直接定义出所有的正规形式，而不需要依赖重写关系。同时，重写关系的技术难以处理 $eta$ 等价关系，以及许多更加复杂的判值相等。
...
程序员一般不太关心正规形式，因为程序的求值不会在有变量的环境下运行。例如 $lambda x bind x + (1 + 1)$ 在运行时不会计算为 $lambda x bind x + 2$，而是等到有参数输入后再做计算。更狭义而言，只有一小部分 “基础” 类型能在运行时提供信息，例如 $Bool$ 类型或者自然数类型 $NN$。在无变量的情况下，这些类型的正规形式非常简单。例如 $Bool$ 类型只有 $"true"$ 与 $"false"$ 两个元素。这类表达式称作#define[典范形式][canonical form]。
...
范畴中论证的一大组成就是#translate[追图][diagram chasing]。这类似初等平面几何学中的#translate[倒角][angle chasing]，即在复杂的图形中利用一系列性质给出角度的连等式。例如，同构的逆态射总是唯一的，或者以下定理，都可利用追图证明。证明留给读者。
#theorem[
  给定三个映射
...
粗略来说，利用某个数学对象构造新的数学对象，往往都构成合适范畴之间的函子。有时候，我们希望构造箭头方向相反的函子，即将 $X -> Y$ 的箭头映射到 $F(Y) -> F(X)$ 的箭头。这称作#define[反变][contravariant] 函子。不过，我们可以避免额外做此定义。只需要考虑范畴 $cal(C)^"op"$，即将 $cal(C)$ 中的箭头方向反转得到的新范畴，即可将反变函子定义为 $cal(C)^"op" -> cal(D)$ 的普通函子。
...
这样的视角可以将许多追图证明变为颇具代数风格的连等式计算。要证明两个对象 $X$ 与 $Y$ 同构，只需要说明 $yo(X)$ 与 $yo(Y)$ 自然同构，而它们的泛性质又可以允许我们将它们展开成别的预层做计算。

== 集合论撷英 <sec:set-theory>

// 假设读者了解基数的基本定义。基数是度量集合元素个数的数学对象。例如每个自然数都是有限基数，而 $aleph_0$ 描述了全体自然数的数量，$frak(c)$ 表示全体实数的数量，等等。以下谈到的基数都指无限基数。

在类型论的研究中使用集合论的原因有三。在讨论比较复杂的类型论时，可以用集合提供直观解释; 集合论的模型研究非常成熟，在比较各种类型论的强度时可以作为公认的标尺; 使用集合论作为元语言研究类型论，可以有效避免目标语言与元语言的混淆。理论上，除了最后一条理由之外，本书中的集合论语言都可以等效替换为类型论。这样得到的理论就相当于集合论中#translate[内模型理论][inner model theory] 的类型论版本。

在类似 ZFC 的集合论中，一切对象都是集合。集合可以包含一些元素，并且完全由其所包含的元素决定。因此我们可以通过列举出集合的所有元素唯一确定这个集合。例如 $nothing = {}$ 是空集，${nothing}$ 是包含一个元素的集合，
#eq($ { nothing, {nothing}, {nothing, {nothing}} } $)
包含三个元素，等等。如果将集合视作有向图的顶点，而当 $x in y$ 时连一条边 $x -> y$，就构成了巨大的有向图。集合论因此也可以视作对一类极其特殊的有向图的结构的研究。有许多不同的有向图都满足集合论的要求，这些有向图就是集合论的模型。

利用集合的结构可以编码任意数学结构，正如利用二进制数据可以编码图片、声音等等。正如编码图片时需要提前商定格式，利用集合编码数学结构时也有多种办法，需要提前选定。例如，有序对 $(x, y)$ 在集合论中一般按 Kuratowski 方案编码为 ${{x}, {x, y}}$。注意这不意味着有序对_本质就是_这种集合，正如 JPEG 与 PNG 格式都可以编码图像，没有哪一个代表了图像的本质一样。

在集合论中，函数可以靠有序对记录它在每一点的值。例如 ${(1, 1), (2, 4), (3, 9)}$ 编码了函数 $f$，满足 $f(1) = 1$，$f(2) = 4$ 与 $f(3) = 9$。这样，如果两个函数 $f$ 与 $g$ 的定义域相同，并且 $f(x) = g(x)$ 总是成立，它们对应的有序对编码就相等。

ZFC 集合论中 C 指代的是*选择公理*，即给定一族集合 $X_(alpha in I)$，如果每个 $X_alpha$ 都有元素，那么存在函数 $f$ 使得 $f(alpha) in X_alpha$ 在每个集合中选出元素。注意如果只有一个有元素的集合 $X$，那么选出 $x in X$ 就是 $exists$ 量词的性质，无需选择公理。对于任何有限个集合，都可以应用 $exists$ 的性质选出元素。但是由于逻辑证明都是有限长的，对于 $I$ 无限的情况就需要额外的公理作出选择。

=== 基数

我们可以依靠一一对应比较集合元素的多少。假如有两个集合 $A$ 与 $B$，可以将二者的元素两两配对，没有遗漏也没有重复，就说 $A$ 与 $B$ #define[等势][equipotent]，或者说其元素数量 $abs(A) = abs(B)$ 相等。#define[基数][cardinal number] 就是描述元素数量的数。对于有限集，元素数量由自然数描述。全体自然数的元素数量写作 $aleph_0$。这也是全体偶数、全体有理数等的元素数量。

对于两个集合 $A$ 与 $B$，可以靠单射或满射比较元素的多少。如果从 $A$ 到 $B$ 有单射，就说 $abs(A) <= abs(B)$; 如果从 $B$ 到 $A$ 有满射或者 $A$ 是空集，就说 $abs(A) scripts(<=)^* abs(B)$。假定选择公理，可以证明这两者等价。同时，可以说明 $abs(A) <= abs(B)$ 与 $abs(A) >= abs(B)$ 同时成立时有 $abs(A) = abs(B)$，并且对于任何两个集合，这三者之一必然成立。这样，全体基数就从小到大排列成一条线。

给定两个基数 $kappa$ 与 $lambda$，取集合的不交并得到的新集合的基数写作 $kappa + lambda$，而取乘积得到的基数是 $kappa times lambda$。对于有限基数而言，这就是自然数的加法与乘法。假如集合 $A$ 与 $B$ 有限，那么从 $A$ 到 $B$ 的函数一共有 $abs(B)^(abs(A))$ 个，因为每个元素 $a in A$ 处的函数值都有 $abs(B)$ 种选择，而一共要选择 $abs(A)$ 次。因此，我们推广到无限基数时，也用函数集定义基数的幂 $lambda^kappa$。

全体实数的基数，与任意非空区间 $(a, b)$、$[a, b]$、$[a, b)$ 等等的基数都相同，写作 $frak(c)$。由于区间 $[0,1]$ 的元素可以写作二进制小数，但是不同的小数可能表示同一个实数，因此有 $frak(c) <= 2^(aleph_0)$。反过来，考虑只含有 $1$ 与 $2$ 的十进制小数，共有 $2^(aleph_0)$ 个，并且两两不同，因此 $frak(c) >= 2^(aleph_0)$。这样就能说明 $frak(c) = 2^(aleph_0)$。

=== 序数

ZF 集合论及其变体中，最重要的结构是良序关系与序数。

#definition[
给定集合 $X$ 与其元素之间的二元关系 $prec$，考虑 $prec$ 的归纳法：对于任何子集 $U subset.eq X$，假如元素 $t in X$ 满足 ${ x in X mid(|) x prec t } subset.eq U$，那么 $t in U$。假如归纳法对于 $prec$ 成立，就说 $prec$ 是*良基关系*。
] <def:well-founded>
对于自然数而言，如果定义 $n prec m$ 当且仅当 $m = n+1$，那么 $prec$ 的归纳法就是普通的数学归纳法。自然数上的小于关系 $<$ 的归纳法则是强归纳法。因此它们都是良基关系。大致上说，良基关系就是不存在无穷下降 $x_1 succ x_2 succ x_3 succ dots.c$ 的二元关系。

ZF 等集合论中的*正则公理*说的就是 $in$ 是良基关系。这从直观上说明任何集合都可以从空集开始通过反复嵌套得到，因此从哲学意义上为集合的产生方式提供了图景.#footnote[需要注意的是这并不能用来避免 Russell 悖论 (见@sec:paradoxes-naive)，因为添加公理_不可能_消除悖论。] 这也导致 ZF 集合论中的许多结构可以靠良基关系描述。

#definition[
如果良基关系 $prec$ 满足如果 $x prec y prec z$，则 $x prec z$，并且任意两个不相同的元素总有 $x prec y$ 或者 $y prec x$ 之一成立，就说 $prec$ 是*良序关系*。
] <def:well-order>

良序关系下的元素不仅不存在无穷下降，而且能从小到大排列成一条线。良基关系则可以出现分叉或者合并的情况。例如 $NN$ 与有限集 ${0,1,dots,n}$ 在小于关系下都构成良序关系。进一步，#eq($ (0,0) prec (0,1) prec (0,2) prec dots.c prec (1,0) prec (1,1) prec dots.c $)
也构成 ${0,1} times NN$ 上的良序关系。类似地，$NN times NN$ 上也有良序关系。

良序关系的重要性质是两个良序关系之间可以相互比较 —— 将两者都从左到右排成一条线，并且靠左对齐，则一者必然能与另一者的前段重合。在前面的例子中， $NN$ 就可以与 ${0,1} times NN$ 的子集 ${0} times NN$ 重合。将同构的良序集视作相等，得到的就是*序数*。$NN$ 对应的序数写作 $omega$，而 ${0,1} times NN$ 写作 $2 omega$，等等。序数可以视作良基关系的 “高度” 的标尺。序数本身几乎也构成良序，唯一的不同是全体序数 $"No"$ 不构成集合。

给定选择公理，任何集合都存在某个二元关系 $<$ 使得它构成良序。不过这样的良序往往极难描述。例如 $RR$ 上的小于关系不构成良序，并且无法给出任何直观的良序关系的定义。每个集合都可能有多种良序关系，例如 $NN$ 上可以定义 $0 < 2 < 4 < dots.c < 1 < 3 < 5 < dots.c$，则这个良序与 $2 omega$ 同构。不难看出某个集合上可能的良序关系的结构只与集合的基数有关。

如果没有选择公理，那么有些基数上不一定能给出良序关系。有良序关系的无限基数称作 *$aleph$ 数*。它们可以从小到大排列，如 $aleph_0 < aleph_1 < dots.c < aleph_omega < dots.c$ 等。在 ZFC 集合论中有选择公理，因此 $frak(c) = 2^(aleph_0)$ 一定是某个 $aleph_alpha$，但是 $alpha$ 具体的值在 ZFC 中无法确定.#footnote[除了 $alpha = 0$ 或者 $alpha = omega$ 等几个少数的特殊值可以证明不可能之外，$alpha$ 取任何序数都与 ZFC 相容。] 著名的连续统假设就是说 $frak(c) = aleph_1$。


=== 大基数理论

依值类型论的模型不可避免涉及集合论与大基数的理论。例如直观上类型可以解释为集合，函数类型对应函数的集合，宇宙类型则是一定大小以内全体集合的集合。然而这行不通。例如在 ZFC 集合论中，就算限定集合只能包含一个元素，全体一元集也无法构成集合 #eq($ V_1 = {y mid(|) exists x bind y = {x}}. $) 假设不然，那么并集 $union.big V_1$ 按照公理也应该构成集合，但这恰好是全体集合的集合，在 ZFC 中不能存在。因此，我们需要发展一套理论，描述 “包含了足够多集合” 的集合。
...
在 ZFC 集合论中，给定基数 $kappa$，前面提到我们无法将全体元素个数少于 $kappa$ 的集合构成集合，这是因为尽管这些集合本身的大小有限制，但是它们的元素可以是任何别的集合，因此在数量上没有限制。假如我们要求不仅这些集合的元素个数少于 $kappa$，并且它们元素的元素个数也少于 $kappa$，元素的元素的元素等等都做如此要求，得到的东西称作#define(key: "hereditarily kappa-small set")[继承 $kappa$ 小集][hereditarily $kappa$-small set] $H_kappa$。例如，${nothing, {nothing}}$ 是继承有限集。可以证明 $H_kappa$ 构成集合。
...
假如存在大于 $aleph_0$ 的强不可达基数 $kappa$，那么 $H_kappa$ 就能构成 ZFC 的模型，故而意味着 ZFC 自洽。由 Gödel 不完备性定理，这意味着 ZFC 不可能证明强不可达基数的存在性.#footnote[除非 ZFC 本身有矛盾，不过那样天就塌了。] 这很合理。例如 Lean 定理证明助手中可以形式化 ZFC 的模型 @lean-con-ch，因此某种意义上说 Lean 的类型论比 ZFC 要强。然而，依值类型论的模型的研究重点并不是讨论各种理论的证明论强度，而在于可计算性、同伦论、程序语义等等。因此，我们不会取 ZFC 为元理论，而是添加足够强的大基数假设，使得我们能专注于对模型其他性质的研究。

在类型论中，往往需要一系列宇宙 $cal(U)_0, cal(U)_1, cal(U)_2, dots$，因此不能仅仅假设一个强不可达基数。为此，我们采用以下公理，为我们提供足够多的强不可达基数。
