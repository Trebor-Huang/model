...
我们可以依靠一一对应比较集合元素的多少。假如有两个集合 $A$ 与 $B$，可以将二者的元素两两配对，没有遗漏也没有重复，就说 $A$ 与 $B$ #define[等势][equipotent]，或者说其元素数量 $abs(A) = abs(B)$ 相等。#define[基数][cardinal number] 就是描述元素数量的数。对于有限集，元素数量由自然数描述。全体自然数的元素数量写作 $alef_0$。这也是全体偶数、全体有理数等的元素数量。
...
全体实数的基数，与任意非空区间 $(a, b)$、$[a, b]$、$[a, b)$ 等等的基数都相同，写作 $frak(c)$。由于区间 $[0,1]$ 的元素可以写作二进制小数，但是不同的小数可能表示同一个实数，因此有 $frak(c) <= 2^(aleph_0)$。反过来，考虑只含有 $1$ 与 $2$ 的十进制小数，共有 $2^(aleph_0)$ 个，并且两辆不同，因此 $frak(c) >= 2^(aleph_0)$。这样就能说明 $frak(c) = 2^(aleph_0)$。
