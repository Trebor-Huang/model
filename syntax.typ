...
先考虑无变量的情况：假如有类型 $A$，集合模型中解释为集合 $X$。我们只需添加函数 $p$，将 $X$ 的元素映射到 $A$ 的语法元素，得到三元组 $(X, A, p)$。这样，每个元素 $x in X$ 都携带了自己对应的语法元素 $tack p(x) : A$。
...
不难得到依值类型的版本。定义*核算结构*为三元组 $(X, Gamma, sigma)$，其中 $X$ 是任意集合，$Gamma$ 是语法语境，$sigma : X -> hom(1, Gamma)$ 是一族代换。其上的依值核算结构为三元组 $(Y, A, p)$，其中 $Y$ 是 $X$ 上的集合族，$A$ 是 $Gamma$ 中的类型，而 $p_x : Y_x -> "Tm"(1, A sigma_x)$ 是一族函数，将集合的元素映射到语法元素。
  ...
  $))
语义类型是依值核算结构，而 $(Y, A, p)$ 的语义元素是有序对 $(y, t)$，其中 $y$ 是元素族，使得 $y_x in Y_x$，并且 $Gamma tack t : A$，满足判值相等 $p_x (y_x) = t sigma_x$。语境扩展是 #eq($ (X, Gamma, sigma) dot (Y, A, p) = (product.co_(x in X) Y_x, thick Gamma dot A, thick (x, y) |-> [sigma_x, p_x]). $)
如上文所述，这个模型的构造主体是集合模型，但添加了一些组件追踪其与语法的联系。

==== $Bool$ 类型
我们需要在核算结构 $(X, Gamma, sigma)$ 上定义 $Bool$ 类型结构。其集合族为常集合族 ${"true", "false"}$，与集合语义相同。类型自然是 $Bool$，而映射 ${"true", "false"} -> "Tm"(1, Bool)$ 将集合的元素 $"true"$ 映射到表达式 $"true"$，将 $"false"$ 映射到 $"false"$。
...
$)
不难发现 $W$ 与集合语义中的定义也相同，并且如果集合语义中 $y$ 映射到语法元素 $t$，$z$ 映射到语法元素 $s$，那么 $(y, z)$ 就对应语法的有序对 $(t, s)$。

==== $Pi$ 类型
与 $Sigma$ 类型类似，其中 $C$ 显然应该改为 $Pi A B$，但 $W$ 则略微复杂。$W_x$ 定义为有序对 $(F, f)$ 的集合，其中 $F in product_(y in Y_x) Z_((x, y))$，而 $f in "Tm"(Gamma dot A, B)$ 是语法元素，使得 $F$ 与 $f$ 有对应关系。具体来说，当 $p_x (y) = t$ 时，有 $q_((x, y))(F(y)) = f[sigma_x, t]$。

不能直接将 $W_x$ 定义为集合中的函数集，是因为有许多函数不能被语法表达 (语法只有可数多元素，而如 $NN -> NN$ 的函数集不可数)，并且那些可以被表达的函数往往有多种表达 (如 $ite(b, "true", "false")$ 与恒等函数在集合语义中相等，但是没有判值相等)。因此，为了保证每个函数都对应一个语法上的函数，需要将 $W_x$ 定义为集合函数与语法函数的有序对。此时 $r_x (F, f)$ 显然定义为 $f$ 即可。

==== 宇宙类型
有了上述例子，读者应当不难类似构造别的类型结构。唯一较困难的是宇宙类型。 (...)

=== 典范性的证明

#proof[
  根据可靠性定理，可以得到从语法到核算模型的解释。而语义中的每个对象都带有一个语法对象。如语义元素 $(y, t)$ 中的 $t$。观察核算模型的定义，用归纳法可以证明，将某个语法对象解释为语义对象，再取回语法对象，与最初的语法对象相同。
