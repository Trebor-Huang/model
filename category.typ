...
- 可以让语境与类型合起来构成新的范畴。具体来说，定义 $integral_cal(C)"Tp"$ 的对象为有序对 $(Gamma; A)$，临时用分号与语境扩展 $(Gamma, A)$ 示区分。从 $(Gamma; A)$ 到 $(Delta; B)$ 的箭头是代换 $sigma : Gamma -> Delta$，满足 $B sigma = A$。这样 $"Tm"$ 就可以视作 $\(integral_cal(C)"Tp"\)^"op" -> Set$ 的预层。
