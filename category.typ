...
因为预层范畴 $Psh(cal(C))$ 本身是局部积闭的，我们可以将这个拉回方翻译成外延类型论的语言。注意这个外延类型论与模型原本对应的类型论没有关系，完全属于元语言。我们将其称作#define[逻辑框架][logical framework]，用#LF[绿色背景]与原本类型论中的表达式区别。
...
可以发现，这与模型定义中的 $"Tm"(Gamma, A) times "Tm"(Gamma, B) tilde.equiv "Tm"(Gamma, A times B)$ 非常类似，但重要的区别是省去了 $Gamma$。这是采用语义语境范畴 $cal(C)$ 的预层的功劳。而利用逻辑框架的语言，则让我们大大减少了操作预层范畴的对象所费口舌。

elaborate the phrase "logical framework"

syntactic vs semantic logical framework (bad name?)
