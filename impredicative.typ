...
#translate[构造演算][calculus of constructions] 是一种只有 $Pi$ 类型与宇宙的类型论。在文献中常被缩写为 CoC，早期文献也作 CC。读者或许知道这类类型系统被 Barendregt 称作#define[纯类型系统][pure type system]，并且构造演算的子系统构成一个立方体，称作 $lambda$ 立方。我们_不认为_这是合适的分类。其中的宇宙有一些表现得比较反直觉，只是为了凑齐规律而添加。它在某些方面分类过于细致，而在另一些方面则不够精细。特别是希望以此为基础添加 $Sigma$ 类型或者更复杂的结构时，无法用纯类型系统的框架描述各种变体。读者可以参阅 Bart Jacobs~@on-cubism 对纯类型系统的批评。
...
$)
注意这里限制了 $C$ 只能处于非直谓宇宙 $*$ 中，并且 $Sigma$ 类型也在 $*$ 里。如果 $A = *$，那么与 F 系统中的 $forall$ 类推，也将这种弱 $Sigma$ 类型称作 $exists$ 类型。事实上，如果加入非直谓强 $exists$ 类型 $(exists X bind P(X)) : *$，并且具备投影函数 $(exists X bind P(X)) -> *$，那么系统就有矛盾 @impredicative-sigma。这说明非直谓宇宙无法编码这种类型。但是，这并不排除可以编码 $A$ 与 $B$ 都在 $*$ 宇宙中的 $Sigma$ 类型 $(x : A) times B(x)$。我们将在 #[@sec:impredicative-sigma]证明也这不可能。
...
$)
最直观的想法是直接将其套用在新增的层级上。这样，就有无限多层非直谓宇宙。不过，这样得到的类型论不自洽。Girard 发现，只要两个非直谓宇宙之间有元素关系，类型论就有矛盾。这就是 Girard 悖论，在@sec:girard-paradox 有详细讲述。
...
这么修改之后，还有没有矛盾呢? 我们只需找到一套模型。根据前文的讨论，我们可以在集合模型中试将最底层的类型解释为至多有一个元素的集合。这样，无论多少个这样的集合取 $Pi$ 类型，得到的结果都还是只有一个元素，不会导致悖论。

不过，这么做有一些困难。理论上宇宙 $*$ 应该只有两个元素，空集和单元素集，但是单元素集不只一个。事实上，根据 #[@sec:set-theory]的讨论， $V_1 = {x mid(|) exists y bind x = {y}}$ 是真类，不是集合。

可以尝试强行选定一个单元素集 $1 = {nothing}$，但是这样与 $Pi$ 类型的构造不兼容。
...
为此，我们可以再修改 $Pi$ 类型的定义。如果陪域满足 $tack B istype_*$，那么 $Pi A B$ 就根据 $B$ 是否总有元素定义为空集 $nothing$ 或者某个固定的单元素集 (例如 ${nothing}$)。如果 $tack B istype_i$ $(i > 1)$，那么就按照一般的办法解释为集合的乘积。这样的确可以构造出 $"CC"_omega$（包括构造演算）的模型。
不过，此时累积性就不成立：临时用 $coerce A$ 显式写出从 $istype_*$ 到 $istype_1$ 的转换，那么这个集合模型中 $coerce(Pi A B) istype_1$ 与 $Pi (coerce A) (coerce B) istype_1$ 就不相等。前者是 ${nothing}$，而后者是 ${{(x, nothing) mid(|) x in A}}$，虽然都有一个元素，但是后者有很多元素的元素。

一方面，我们可以接受此事。从 Curry–Howard 对应的角度来看，这就相当于认为非直谓宇宙 $"Prop"$ 的 $forall$ 命题与直谓宇宙的 $Pi$ 类型并不完全是同一种东西，而是只有同构关系。另一方面，我们也可以修改集合模型中函数的编码。不将 $f$ 编码为 ${(x, f(x)) mid(|) x in A}$，而是编码为 ${(x, u) mid(|) x in A, u in f(x)}$。这就是 *Aczel 编码*。注意由于 ZF 集合论中一切对象都是集合，$f(x)$ 也可以看作集合，因此 $u in f(x)$ 合法。
...
我们自然可以向 $"Prop"$ 添加公理，要求该宇宙中的所有类型都至多有一个元素。这称作#define[证明无关性][proof irrelevance]。也可以向 $"hProp"_cal(U)$ 添加公理增加非直谓性，例如要求 $"hProp"_cal(U) tilde.eq "hProp"_(cal(U)')$ 对任何两个宇宙 $cal(U)$ 与 $cal(U)'$ 都成立。这样取直谓 $Pi$ 类型抬升了宇宙层级之后总可以通过此等价降回来。有若干定理给出 $"Prop"$ 与 $"hProp"$ 分别添加对应的公理之后等价。
...
- 大归纳类型有一部分从语法上可以显然判定至多有一个元素，称作#translate[亚单元素类型][subsingleton type]。
...
起初，构造演算的村子里有个非直谓宇宙。人们用它可以构造各种各样的类型。有些人编写程序，有些人构造证明，生活十分幸福。
...
Martin-Löf 在 1971 年提出了最早的 Martin-Löf 类型论，其中有 $cal(U) : cal(U)$ 的规则，看上去可以照搬 Russell 悖论导出矛盾。但是这个类型论不含 $Sigma$ 类型与归纳类型。论文中写作 $Sigma$ 的类型是用 $Pi$ 作非直谓编码的弱 $Sigma$ 类型。因此 Russell 悖论并不能直接套用。1972 年，Girard 才发现这个系统不自洽，见@sec:girard-paradox.//事实上，Martin-Löf 在论文中证明了它是自洽的，但是由于证明的元理论本身满足 $VV in VV$，所以实际上元理论也有矛盾。
...
这则悖论可以做一些简化，避免用到序数的全套理论。例如可以只用良基关系 (@def:well-founded)。假如良基集合 $A$ 与 $B$ 的某个真子集同构，就说 $A < B$。这样全体良基集合的集合 $"WF"$ 也良基。进一步分析可以得到 $"WF" < "WF"$，矛盾。这是 *Мириманов 悖论*。还可以定义无挠序的概念：
#definition-appendix[
...
1972年，逻辑学家 Jean-Yves Girard (注意不是哲学家 René Girard) 在博士论文~@girard-paradox 中研究了一类用于表达高阶逻辑的类型论，其中只包含 $Pi$ 类型与不同的宇宙层级。在附录中，他提出了这些类型论的一种自然推广，起名为 U 系统，并且证明了这个系统不自洽。
...
Girard 在 U 系统中将 Burali-Forti 悖论的简化版本 (即利用无挠序的版本) 形式化，证明了 U 系统中类型 $bot = (X : *) -> X$ 有元素。它自然不可能有正规形式，因为 $bot$ 没有正规元素。因此这也说明 U 系统不满足正规化。Coquand 在 1986 年另给出一版本，利用了良基关系的悖论。这一悖论用到的 $Pi$ 类型种类更少，因此删去不需要的规则之后得到 #Uminus 系统也不自洽。

1995 年，Hurkens 再进一步给出了简化。由于这一版本已经非常不同，而且极其简短，有时改称其为 *Hurkens 悖论*。这利用了非直谓性编码集合 $X$，满足 $X$ 与其双重幂集 $(X -> *) -> *$ 有双射。Hurkens 的论文~@hurkens-paradox 中讨论了这一系列悖论的演化。

尽管 U 系统不自洽，无法作为逻辑系统使用，但它的所有类型都可正规化，因此类型检查仍然可判定。与之相反，$cal(U) : cal(U)$ 的系统中类型检查不可判定。另外，所有这些系统都仍是#define[等式自洽][equationally consistent] 的，即并非所有表达式都判值相等。
...
一般的选择公理说的是，给定类型族 $B(x) istype$ 与谓词 $P : (x : A) -> B(x) -> "Prop"$，如果命题 $forall (x : A) bind exists (y : B(x)) bind P(x, y)$ 成立，那么就存在函数在每个 $B(x)$ 中选出元素，即
