...
一个想法是强行选定一个单元素集 $1 = {nothing}$，但是这样与 $Pi$ 类型的构造不兼容。
...
一方面，我们可以接受此事。从 Curry–Howard 对应的角度来看，这就相当于认为非直谓宇宙 $"Prop"$ 的 $forall$ 命题与直谓宇宙的 $Pi$ 类型并不完全是同一种东西，而是只有同构关系。另一方面，我们也可以修改集合模型中函数的编码。不将 $f$ 编码为 ${(x, f(x)) mid(|) x in A}$，而是编码为 ${(x, u) mid(|) x in A, u in f(x)}$。这就是 *Aczel 编码*。注意由于 ZF 集合论中一切对象都是集合，$f(x)$ 也可以看作集合，因此 $u in f(x)$ 是合法的。
