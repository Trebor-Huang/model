...
我们先证明它们逐点相等，即类型 #eq($ product_(x : P) f(x) = g(x) $) 有元素。这只需要说明 $"Tm"((x : P), f(x) = g(x))$ 有元素即可。根据正常元素族的定义，我们需要当 $x = star$ 时选择 $f(x) = g(x)$ 的正常元素，并且 $x = epsilon$ 时选择 $f(x) = g(x)$ 的任意元素。前者可以选择自反性 $"refl"$，而后者则可以直接选择 $star$。
...
#eq($ product_(f, g : X -> Y) [product_(x : X) f(x) = g(x)] -> (f = g) $)
