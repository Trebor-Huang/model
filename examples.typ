...
对于 $Pi$ 类型来说，我们先讨论如何合理地构造有向图之间的 “映射图” 的概念。给定有向图 $Gamma$ 与 $Delta$，我们定义 $Gamma -> Delta$ 的顶点集为 $Gamma$ 的顶点到 $Delta$ 的顶点之间的映射构成的集合。给定两个这样的映射 $f$ 与 $g$，它们之间的边则需要为 $Gamma$ 的每条边 $x xarrow(e) y$ 选择一条边 $f(x) --> g(y)$。这种构造或许有些莫名其妙，但是它在逻辑学中是 Kripke 模型的变体，而在范畴论中则是预层范畴的指数对象。另一方面，这种构造是唯一能使得模型构造成功的办法，在 (...) 中会更详细地阐述它的来由。

- Pi

最后，对于宇宙类型，我们先忽略集合大小问题。假设 $bullet$ 代表只有一个顶点的有向图，而 $bullet -> bullet$ 代表恰好有一条边的有向图。可以将宇宙有向图 $U$ 的顶点集定义为 $bullet$ 上的全体依值有向图的集合，而边集定义为 $bullet -> bullet$ 上的全体依值有向图的集合。换句话说， $U$ 的顶点集是全体集合的集合，而一条边则是三个集合 $X$、$Y$ 与 $E$，配上两个函数 $E -> X$ 和 $E -> Y$。显然，这条边的两个端点分别是 $X$ 和 $Y$。读者可以验证，图同态 $Gamma -> U$ 与 $Gamma$ 上的依值有向图一一对应。因此，我们可以将宇宙类型 $cal(U) in "Tp"(Gamma)$ 解释为 (...)

接下来与集合模型类似处理集合大小问题。前面提到将语境解释为有向图，实则是解释为 $H_kappa$ 中的有向图，而后我们再取强不可达基数 $lambda < kappa$ 构造宇宙的解释。
我们定义宇宙有向图 $U_lambda$ 的顶点集是 $H_lambda$，而两个顶点 $X$ 和 $Y$ 之间的边则是 $H_lambda$ 中的集合 $E$，配备两个函数 $s : E -> X$ 与 $t : E -> Y$。
...
回忆在有向图模型中，类型是依值有向图，而构造一个元素需要在语境有向图的每个顶点上选择依值顶点，在每条边上选择依值边。那么，假如我们证明语境 $Gamma = (A : cal(U), p : "isProp"(A))$ 的某个顶点上没有任何依值顶点，就可以证明这个类型没有元素。

要选择

(...)

预层模型满足排中律当且仅当这个代数结构中所有操作都有逆。
