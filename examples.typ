...
根据宇宙类型的构造，$"Tm"((), cal(U))$ 的元素应当与有向图一一对应。我们考虑单点有向图 $A = bullet$，$"El"(A)$ 则是对应的空语境中的依值有向图。接下来需要计算 $"El"(A) -> Empty$。根据 #[@sec:graph-exponential]的讨论，我们只需计算单点图到空图之间的映射图。展开定义可以立刻得到这个图也是空的。因此 $"El"(A) + ("El"(A) -> Empty)$ 同构于 $"El"(A)$。但是，$"El"(A)$ 没有元素! 这是因为 $A$ 只有顶点而没有边，而空语境对应有向图 #terminal-graph，有一个自环。因此 $"El"(A)$ 的元素需要选出自环上的依值边，而这是不可能的。这样就说明了有向图模型中排中律不成立，进而说明 Martin-Löf 类型论中无法证明排中律。
...
我们不仅可以使用纯类型论的语言，还可以加入有向图中特有的对象与公理。例如上文构造了单点有向图对应的类型 $A$。在空语境中可以计算函数类型 $A -> B$ 对应的有向图，其顶点与 $B$ 的顶点一一对应，而任意两个顶点之间恰好有一对有向边 $(bullet arrows.lr bullet)$。换言之，$A -> B$ 总是完全图。另一方面，$A times B$ 的顶点也与 $B$ 的顶点一一对应，而没有任何有向边。因此 $A times B$ 是对应的离散图。

由此，我们可以构造更复杂的命题。例如 $lambda b bind lambda a bind b : B -> (A -> B)$ 是从图 $B$ 到对应的完全图的含入映射。如果这个映射构成同构 $B tilde.equiv (A -> B)$，那么就可以说 $B$ 是完全图。这样就完全用类型论的语言定义了某个有向图是否是完全图。

由前一节的计算可知，$A -> Empty$ 是空的，因此 $(A -> Empty) -> Empty$ 就与单元素类型 $Unit$ 对应的图同构。因此我们可以加入一条公理 $((A -> Empty) -> Empty) tilde.equiv Unit$。这样就能导出关于完全图等等的更多性质。在内语言中，这意味着 $A$ 作为命题满足其双重否定为真，但本身不为真。这就凸显了许多综合数学系统不假设排中律的原因：在有向图的 “数学宇宙” 中，命题编码了一个图的子图在每个局部可能出现的情况 (因为 $G$ 的子图应能写成 $Sigma$ 类型 $sum_(x : G) P(x)$，其中 $P(x)$ 是命题)。例如子图可以包含顶点但不包含其间的边，等等。因为有了这些丰富的情况，所以不能假设排中律。
...
注意到以上证明中用到了类型的类型 $cal(U)$。这是依值类型论中归纳子额外强度的来源。事实上，如果类型论中没有任何宇宙，是无法证明 $0 != 1$ 的。要说明这一点，只需要找到某个模型使得空语境下类型 $0 != 1$ 没有元素。我们的思路是构造某个模型使得 $0$ 与 $1$ 的解释相等，此时类型 $0 = 1$ 自然就有元素。如果还能保证空类型无元素 —— 即没有矛盾 —— 就说明 $0 != 1$ 一定没有元素。
...
此时，语境扩展也可以分类讨论定义。当 $Gamma = varnothing$ 时定义 $(Gamma, A) = varnothing$; 当 $Gamma = {star}$ 时令 $(Gamma, A) = A_star$ 即可。注意这里与集合模型不同：集合模型中 $(Gamma, A)$ 的解释是 ${(x, a) mid(|) x in Gamma, a in A_x}$，如果套用到这里就得到集合 ${(star, star)}$。但是我们希望语境的解释只能是 $varnothing$ 与 ${star}$，不能是 ${(star, star)}$，因此这里需要手动修改。
...
在这个模型中，由于 $0$ 与 $1$ 的解释相同，因此类型 $0 = 1$（即 $"Id"(NN, 0, 1)$） 有元素。然而空类型在空语境下没有元素，因此函数类型 $(0=1) -> Empty$ 也没有元素。这就说明无宇宙的 Martin-Löf 类型论中无法证明 $0 != 1$。
...
类型构造的思路是，对于不含 $eta$ 等式的类型 $A$，我们与集合模型的构造相比额外添加一个值作为 $"error"(A)$ 的定义。对于含有 $eta$ 等式的类型，比如 $A times B$，则不作添加，直接定义 $"error"(A times B)_x = ("error"(A)_x, "error"(B)_x)$。这是因为如果额外添加错误值，那么这个新的元素就无法满足 $eta$ 等式。某种意义下，这是#translate[惰性求值][lazy evaluation] 的错误值。
...
#definition[
  给定群胚 $Gamma$ 上的群胚族 $A$，定义 $A$ 的元素族 $a in "Tm"(Gamma, A)$ 为每个点 $x in Gamma$ 选择元素 $a_x in A_x$，为每条道路 $p in hom_Gamma (x, y)$ 选择道路 $a_p in hom_A_y (A_p (a), b)$，使得 $a_refl = refl$ 与 $a_(p*q) = a_p * A_p (a_q)$ 成立。这些等式无非是让 $a$ 保持群胚运算，不过因为依值类型的原因需要插入映射 $A_p$ 进行转换。
]
...
在此翻译下就是可证的。然而，通常数学中需要用到选择公理的命题，如所有向量空间都有基等，反而不能用这个翻译证明。这说明将 $exists$ 命题翻译为 $Sigma$ 类型不是合理的方案。
...
- Discuss various notions of choice @sec:berardi-paradox
