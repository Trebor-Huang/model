...
空类型不难看出应该解释成空的依值有向图。由于空集到任何其他集合都恰好有一个函数，不难定义出所需的消去子。与集合模型类似，#[@sec:empty-type]中提到的空类型 $eta$ 等式也成立。
...
尽管有向图模型只是为了演示模型构造的办法，但是它也有一定用处。有向图模型构成排中律的反模型。如果采用 “类型就是命题” 的理解办法，那么排中律是
#eq($
