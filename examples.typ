...
这里， $"error"(A)_x$ 为每个类型赋予异常。直观上，应该要求它们的确是异常值，即 $"error"(A)_x in.not overline(A)_x$。不做此要求的理由在之后会解释。因此，需要区分异常值 (即不属于 $overline(A)_x$ 的元素) 与异常元素 $"error"(A)_x$。
...
对于最简单的单元素类型，我们定义 $"Unit"_x = {star}$ 为单元素集合，$overline("Unit")_x = {star}$ 为全体元素。根据之前的讨论，单元素类型选定的异常元素按 $eta$ 等式必须是 $star$。这意味着 $"error"(A)_x$ 有可能在 $overline(A)_x$ 中，是正常值。注意我们也不能将 $star$ 改为异常值，即将 $overline("Unit")_x$ 定义为空集。因为这样 $"Tm"(Gamma, "Unit")$ 也是空集，不合要求。
...
在定义 $"Bool"$ 类型时，我们额外添加一个异常元素 $epsilon$。当消去子 $ite(b, t, f)$ 遇到这个异常元素时，就返回 $"error"(A)$，其中 $A$ 是消去子的返回值。因为没有 $eta$ 等式，所以这样定义不会破坏需要的性质。

$Pi$ 类型也具有 $eta$ 规则，因此不额外添加异常元素。$Pi A B$ 的定义与集合模型中的定义一样，是集合 #eq($ (Pi A B)_x = product_(a in A_x) B_((x, a)). $) 其中正常元素 $f in \(overline(Pi A B)\)_x$ 是那些将正常元素 $a in overline(A)_x$ 映射到正常元素 $f(a) in overline(B)_((x, a))$ 的函数。将异常元素 $"error"(Pi A B)_x$ 定义为函数 $a |-> "error"(B)_((x, a))$，也就是将所有 $A$ 中的元素都映射到 $B$ 的异常元素。读者熟悉构造时，可以试着考虑 $B$ 不依赖 $A$ 时，即普通函数类型的特殊情形是怎么样的。

相等类型没有 $eta$ 规则，因此我们额外添加异常元素。具体来说，
...
构造主义往往与可计算的事物联系在一起。这是因为构造主义中的原理都可以找到程序对应。这就是 Brouwer–Heyting–Kolmogorov 解释。相反，排中律就难以找到类似的程序.#footnote[如果拓展我们对程序的定义，那么仍然可以找到#translate[计算续体][continuation] 等解释。但是 “一个程序要么停机要么不停机” 无论如何也不可能解释出可以判定停机性的程序，因此这样的拓展是有限度的。] 不过需要提醒读者的是，BHK 解释并不是声称_所有_能找到程序对应的逻辑原理_都属于_构造主义。事实上，有一些逻辑原理虽然有程序对应，但我们一般不认为是纯粹构造的。
...
为此，我们先来抽象定义计算模型的概念。由于计算模型中往往会出现不停机、出错等情况，因此会涉及#translate[偏函数][partial function]，即只在定义域的某个子集上有值的函数。如果如果某个表达式 $f(x)$ 有定义，就写 $f(x) defined$，否则写 $f(x) diverge$。以下如果有表达式之间的等式 $f(x) = g(y)$，就表示二者要么同时无定义，要么同时有定义并且相等。举个例子，按照这样的惯例不能写 $x\/x = 1$，因为 $x = 0$ 时左边无定义但右边有定义。
#definition[
  #define[偏组合代数][partial combinatory algebra] 由一个集合 $A$ 与偏二元运算 $A times A harpoon A$ 组成，直接写作左结合的并列或者点乘符号，即 $x y z = x dot y dot z = (x y) z$。
  
  二元运算需要满足#define[组合完备性][combinatorial completeness]：对于任何 $n$ 个变量的组合，如 $x y y (z y)$ 或者 $z (w (x y)) z$ 等等，都存在某个元素 $a in A$，使得 $a x_1 ... x_n$ 等于这个组合。
]
这里，$A$ 的元素既是程序又是数据，$f x$ 表示将程序 $f$ 输入数据 $x$ 运行.#footnote[也有具备类型系统的偏组合代数，可以将程序与数据分开，读者可自行了解。] 组合完备性保证了存在足够多的程序，供我们对数据做基本的操作。例如存在 $k in A$ 使得 $k x y = x$，因此 $k x$ 可以作为常函数。
...
另一个例子是 $lambda$ 演算。考虑 $beta$ 等价意义下的表达式集合 $Lambda$，二元运算就是函数应用。这样组合完备性显然成立，例如 $k x y = x$ 可以直接由 $lambda x y bind x$ 表达。值得注意的是，这个二元运算是全运算，因为不停机的表达式也是集合 $Lambda$ 中的元素。如果我们只考虑#define[既约][reduced] 表达式 $Lambda'$，即无法 $beta$ 归约的表达式，那么二元运算就会变成偏运算。

有了计算模型的定义，就可以着手定义哪些数学对象是可计算的。以下我们固定一个具体的偏组合代数 $A$。
#let realizes = math.scripts(sym.forces)
#definition[
  如果有集合 $X$ 与二元关系 $r realizes x$，表示 “程序 $r in A$ 实现了元素 $x in X$”，使得每个元素都至少有一个程序实现，就称它为#define[汇编][assembly]。如果一个程序至多实现一个元素，就称之为#define[谦][modest] 集合。
]
在每个计算模型中，都会有自然数的某种实现。例如 $lambda$ 演算中的 Church 自然数，在图灵机中用二进制纸带表达自然数，等等。这样就可以定义 $NN$ 上的汇编关系 $realizes_NN$。

// TODO too wordy

#definition[
  给定两个汇编 $X$ 与 $Y$，函数 $f : X -> Y$ *可计算*当且仅当存在程序 $r in A$ 使得 $a realizes_X x$ 时总有 $r a realizes_Y f(x)$。
]

假如偏组合代数 $A$ 是 Turing 机或者 $lambda$ 演算，那么函数 $f : NN -> NN$ 在此意义下可计算，当且仅当它在通常的定义下可计算。因此这个定义是合理的推广。

这些定义非常朴素，但是点出了可计算性的讨论中编码的重要性。我们在讨论可计算性时提到的自然数实际上是带有 $realizes_NN$ 结构的，只不过隐去不谈。假如我们改用停机的 $lambda$ 表达式编码 $0$，用不停机的 $lambda$ 表达式编码 $1$，这样得出来的可计算函数就与正常的定义大相径庭。在讨论更复杂的对象的可计算性时，编码就尤为重要。例如数值计算多项式的零点时，输入的是多项式的系数，还是一个黑盒程序 (保证这个程序正确实现了某个多项式)，所需要的技术完全不同。
