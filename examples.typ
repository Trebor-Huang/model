...
根据宇宙类型的构造，$"Tm"((), cal(U))$ 的元素应当与有向图一一对应。我们考虑单点有向图 $A = bullet$，$"El"(A)$ 则是对应的空语境中的依值有向图。接下来需要计算 $"El"(A) -> Empty$。根据 #[@sec:graph-exponential]的讨论，我们只需计算单点图到空图之间的映射图。展开定义可以立刻得到这个图也是空的。因此 $"El"(A) + ("El"(A) -> Empty)$ 同构于 $"El"(A)$。但是，$"El"(A)$ 没有元素! 这是因为 $A$ 只有顶点而没有边，而空语境对应有向图 #terminal-graph，有一个自环。因此 $"El"(A)$ 的元素需要选出自环上的依值边，而这是不可能的。这样就说明了有向图模型中排中律不成立。根据可靠性 (@thm:soundness)，假如 Martin-Löf 类型论中排中律这个类型有元素 $t$，那么有向图模型中也会有对应的元素 $bracket.stroked(t)$，矛盾!
...
这里， $bot_(A, x)$ 为每个类型选出了一个*发散值*。注意与异常值相区分：前者是单个元素，而后者可以有多个。直观上，应该要求发散值属于异常值，即 $bot_(A, x) in.not overline(A)_x$。不做此要求的理由在之后会解释。
...
这个类型 $P$ 可以看作_利用归纳类型定义_的单元素类型，因为归纳类型没有 $eta$ 规则。需要注意的是，有些证明助理为了方便，将恰有一个构造子的归纳类型视作带有 $eta$ 的#translate[记录类型][record type]。但这两者在理论上是满足不同规则的。
...
==== $Pi$ 类型
$Pi$ 类型也具有 $eta$ 规则，因此不额外添加发散值。$Pi A B$ 的定义与集合模型中的定义一样，是集合 #eq($ (Pi A B)_x = product_(a in A_x) B_((x, a)). $) 其中正常元素 $f in \(overline(Pi A B)\)_x$ 是那些将正常元素 $a in overline(A)_x$ 映射到正常元素 $f(a) in overline(B)_((x, a))$ 的函数。将发散值 $bot_(Pi A B,x)$ 定义为函数 $a |-> bot_(B, (x, a))$，也就是将所有 $A$ 中的元素都映射到 $B$ 的发散值。读者可以试着考虑 $B$ 不依赖 $A$，即普通函数类型的特殊情形是怎么样的。
