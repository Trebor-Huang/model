...
$Sigma$ 与 $Pi$ 类型与有向图十分类似。有群胚 $Gamma$，依值群胚 $A$ 与 $integral A$ 上的依值群胚 $B$，我们需要定义 $Gamma$ 上的依值群胚 $Sigma A B$ 与 $Pi A B$。前者非常直观，定义 $x in Gamma$ 上的依值点集为 ${(a, b) mid(|) a in A_x, b in B_((x, a))}$，即点的有序对。道路集也是道路的有序对。$Pi$ 类型则需要增加一些条件。 $Pi A B$ 在 $x in Gamma$ 上的依值点是映射 $f$，不仅需要将 $a in A_x$ 映射到 $B_((x, a))$，还需要将依值道路 $p in A_refl(x)$ 映射到 $B_((refl(x), p))$ （...）



给定依值群胚 $A$ 与两点 $a, b in A_x$，它们之间有道路集合 $hom_A^refl(x) (a, b)$。我们希望将其作为相等类型的语义。但相等类型本身也应该是群胚，包含道路之间的道路信息。由于依值群胚 $A$ 没有记录二维信息，我们需要为集合 $hom_A^refl(x) (a, b)$ 补上平凡的道路。
#definition[
  给定集合 $X$，定义*离散群胚*的点集是 $X$，只有平凡道路 $refl(x)$。集合 $X$ 对应的离散群胚也写作 $X$。同理，给定群胚 $Gamma$ 点集上的集合族 $A$，补上所有的平凡道路 (???) 也可以构成 $Gamma$ 上的依值群胚，称作*离散依值群胚*。
]
离散群胚之间的同态 $X -> Y$ 恰好是集合之间的映射 $X -> Y$。因此，群胚模型完整包含了一份集合模型。读者可以验证群胚模型中的所有类型构造限制在离散群胚中就恰好与集合模型中的构造相同。
