...
这个定义简单得令人惊讶。读者或许期望另有条件，要求存在某个程序 $r in AA$ 实现了这个汇编族。究其本质，是因为类型信息在程序中是完全擦除的，因此不需要也不能有额外的要求以定义某种 “可计算汇编族” 的概念。
...
#definition[
  给定 $Gamma$ 上的汇编族 $A_x$，它的不交并 $integral A$ 是汇编。底集合为 $product.co_(x in Gamma) A_x$，而 $r realizes (x, a)$ 当且仅当 $r = P r_1 r_2$，其中 $r_1 realizes x$，并且在汇编 $A_x$ 中 $r_2 realizes a$。
]
#lemma[
