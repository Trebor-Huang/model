...
由前一节的计算可知，$A -> Empty$ 是空的，因此 $(A -> Empty) -> Empty$ 就与单元素类型 $Unit$ 对应的图同构。因此我们可以加入一条公理 $((A -> Empty) -> Empty) tilde.equiv Unit$。这样就能导出关于完全图等等的更多性质。在内语言中，这意味着 $A$ 作为命题满足其双重否定为真，但本身不为真。这就凸显了许多综合数学系统不假设排中律的原因：在有向图的 “数学宇宙” 中，命题编码了一个图的子图在每个局部可能出现的情况。例如子图可以包含顶点但不包含其间的边，等等。因为有了这些丰富的情况，所以不能假设排中律。
...
在这个模型中，由于 $0$ 与 $1$ 的解释相同，因此类型 $0 = 1$（即 $"Id"(NN, 0, 1)$） 有元素。然而空类型在空语境下没有元素，因此函数类型 $(0=1) -> Empty$ 也没有元素。

此模型中不能加入宇宙，因为 $Empty$ 与 $Unit$ 两个类型的解释不相等。一旦加入宇宙，由于我们要求 $"Tm"(Gamma, A)$ 至多有一个元素，宇宙 $"Tm"(Gamma, cal(U))$ 也只能有一个元素。这意味着没办法定义 $"El" : "Tm"(Gamma, cal(U)) -> "Tp"(Gamma)$，使得它的取值同时包含 $Empty$ 与 $Unit$。这就说明无宇宙的 Martin-Löf 类型论中无法证明 $0 != 1$。
