...
$Pi$ 类型则有些许复杂，不过同样可以利用有向图 #[@sec:graph-exponential]中提到的技巧计算。具体而言，$(Pi A B)_x$ 的依值顶点 $f$ 应当是映射，将 $a in A_x$ 映射到 $f(a) in B_((x, a))$，将 $alpha in hom_A^(refl(x)) (a_1, a_2)$ 映射到 $f(alpha) in hom_B^((refl(x), alpha)) (f(a_1), f(a_2))$。使得其保持道路的各种运算。而给定道路 $p in hom_Gamma (x, y)$，依值道路 $phi in hom_(Pi A B)^p (f_1, f_2)$ 应该将每个 $alpha in hom_A^p (a_1, a_2)$ 映射到依值道路 $hom_B^((p, alpha)) (f_1 (a_1), f_2 (a_2))$，同样需要保持道路的运算。直观上来说，群胚模型中的函数不仅要描述每个点的映射，还要描述每条道路的表现，也就是手动给出了
#eq($ (a = b) -> (f(a) = f(b)) $)
...
给定依值群胚 $A$ 的两个元素 $s$ 与 $t$，定义依值群胚 $"Id"(A, s, t)$ 在 $x in Gamma$ 上的依值点集为 $hom_A^refl(x) (s_x, t_x)$。对于依值道路来说，给定道路 $p in hom_Gamma (x, y)$ 与 $mu in hom_A^refl(x) (s_x, t_x)$ 和 $nu in hom_A^refl(y) (s_y, t_y)$，我们需要定义依值道路集 $hom_("Id"(A, s, t))^p (mu, nu)$。有正方形
#eq(diagram($
  s_x edge(mu) edge("d", s_p) & t_x edgeL("d", t_p) \
  #box($s_y$, baseline: -0.15em) edgeR(nu) & #box($t_y$, baseline: -0.15em)
$))
