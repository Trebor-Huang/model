...
将类型解释为空间，最重要的意义是支撑#translate[泛等公理][univalence]。大致来说，两个类型的名字之间相等 $"Id"(cal(U), A, B)$，就能给出类型之间的等价 $"El"(A) tilde.eq "El"(B)$。泛等公理声称这反过来也成立：任何类型之间的等价都来自于它们名字之间的相等关系。这意味着在宇宙 $cal(U)$ 里，_任何类型都至多有一个名字_。由于 Russell 悖论等问题，不能要求任何类型都恰好有一个名字，因此泛等公理就是退而求其次的要求。
