...
从同伦论的视角，泛等公理使得类型论可以作为研究空间的*综合*语言。即无需从 ZFC 集合开始构造出空间的概念，而是直接将其嵌入形式系统中，作为语言的一部分。这样可以给出同伦论中许多命题的优雅证明。平面几何的公理系统也是一种综合语言，直接在形式系统中描述点与线的关系，与解析几何利用实数表达几何概念相对.#footnote[不过，平面几何属于公理语言，即以一般的逻辑为基础，加入若干公理刻画数学对象的性质。综合语言不全是公理语言，例如同伦类型论或综合微分几何就不是公理语言。] 在 #[@sec:graph-synthetic]中有对综合数学基本原理的介绍。
